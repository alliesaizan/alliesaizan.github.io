---
title: Teardrops on my Data Frames
author: ~
date: '2018-07-25'
slug: taylor-swift-song-lyrics
categories: []
tags: []
subtitle: ''
---
...
dat <- read.csv("/Users/alliesaizan/Documents/R Tinkering/TSwift Lyrics/taylor_swift_lyrics.csv")
dat$lyric <- as.character(dat$lyric)
data("stop_words")

```

### A text analysis of Taylor Swift's discography

``` {r image, out.width = "200px", echo = FALSE}
knitr::include_graphics("/Users/alliesaizan/Documents/R Tinkering/TSwift Lyrics/Taylor_Swift_Cavendish.jpeg")
```
...
I thought it might be cool to look for patterns in Taylor's music. My running theory for the past decade is that Taylor's songs all sound the same. Would the data bear that pattern out?
...
I will treat each song as a "document." Before we dive into the analysis, let's look at the most frequent songs in her collection. 
...
``` {r lda, include = FALSE}
k <- 5 
ldaOut.terms <- read.csv(file=paste("/Users/alliesaizan/Documents/R Tinkering/TSwift Lyrics/LDAGibbs_k=",k,"_TopicsToTerms.csv"))
...
ldaOut.topics <- read.csv(file=paste("/Users/alliesaizan/Documents/R Tinkering/TSwift Lyrics/LDAGibbs_k=",k,"_Topics.csv"))
topics <- data.frame(ldaOut.topics)

  # Merge topics by song category to main data frame
datV2 <- cbind(datV2, topics)
datV2$V1 <- as.factor(datV2$V1)
